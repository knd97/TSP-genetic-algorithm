...
void FileHandler::filesInDirectory(const std::filesystem::path &dir) const
{
...
bool FileHandler::checkPath() const
{
	try
	{
		const std::filesystem::path directory{ getFileName() };
		std::cout << "Current working directory: " << std::filesystem::canonical(directory).string() << "\n";
